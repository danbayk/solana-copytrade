...
const myAccount = new web3.PublicKey(config_1.walletAddress);
...
async function getMarketInfo(marketId){
    const marketInfo = await connection.getAccountInfo(marketId, "processed");
    return marketInfo;
}
...
async function getMintData(mint){
    return await connection.getAccountInfo(mint, "processed");
}
